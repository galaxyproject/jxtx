...
function ButtonSocial(props) {
  ...
  const classButtonSize =
    ButtonClassname[`${ButtonType.SOCIAL}_${ButtonSize[iconSize]}`];
  const classButtonType = ButtonClassname[ButtonType.SOCIAL];
  const classNamesButton = classNames(
    buttonStyles[classButtonType],
    buttonStyles[classButtonSize]
  );

  return (
    <a className={classNamesButton} href={attributeHREF} target={Target.BLANK}>
      {children}
    </a>
  );
}
