...
function Tile(props) {
    ...
    const classOrientation = TileClassname[orientation] || TileClassname[TileOrientation.VERTICAL];
    ...
    return (
        <div className={compStyles[classOrientation]}>
            ...
            {React.Children.map(children, child => React.cloneElement(child, {scale: contentScale}))}
        </div>
    )
}
