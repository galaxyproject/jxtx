...
function TileHeading(props) {
  ...
  const classTheme =
    scale === TileContentScale.MEDIUM ? HeadingClassname.ORANGE : null;
  const classNamesHeading = classNames(
    headingStyles[classScale],
    headingStyles[classTheme]
  );

  return <h3 className={classNamesHeading}>{children}</h3>;
}
