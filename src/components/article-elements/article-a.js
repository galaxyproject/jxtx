...
function ArticleA(props) {
    ...
    const target = props.target || Target.BLANK; /* Required for inline <a/> elements. For example a link inside a <figcaption/>. */

    return (
        children ?
            internal ?
                <Link className={compStyles.article__link} to={href}>{children}</Link> :
                <a className={compStyles.article__link} target={target} {...props}>{children}</a> :
            null
    )
}
